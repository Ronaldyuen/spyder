...
""" 
Notes:
- This version is based on the latest 5.x, should be able to start normally following the spyder env setup.
...
- Adding Filter box at the columns header, mainly used for filtering rows 
    - query instructions, such as equality and comparison operators are documented in _get_query_list
- reset action available when right click on a cell, which could reset the filtered dataframe, while retaining selected position
- coloring function is improved and supports when a column only contains a few unique items
- navigation is added for 
    - scroll to the end (row/col) of the dataframe
    - scroll up/down to a cell that is different in value at this column
...
class DataFrameModelExtra:
    ...
    def __init__(self, df: pd.DataFrame):
        self.original_df = df
        # decides to load in block and color or not
        self.LARGE_SIZE = 5e5
        self.LARGE_NROWS = 1e5
        self.LARGE_COLS = 60
        # initial loading block and increment, but overridden when scroll is used
        self.ROWS_TO_LOAD = 500
        self.COLS_TO_LOAD = 40  # ideally should load more than the width
...
class DataFrameModel(QAbstractTableModel):
    ...
    def __init__(self, dataFrame, format=DEFAULT_FORMAT, parent=None):
        ...
        if size < self.model_extra.LARGE_SIZE:
        ...
        if size > self.model_extra.LARGE_SIZE:
            ...
            if self.total_rows > self.model_extra.LARGE_NROWS:
                self.rows_loaded = self.model_extra.ROWS_TO_LOAD
            else:
                self.rows_loaded = self.total_rows
            if self.total_cols > self.model_extra.LARGE_COLS:
                self.cols_loaded = self.model_extra.COLS_TO_LOAD
            else:
    ...
    def header(self, axis, x, level=0):
        ...
        if not hasattr(ax, 'levels'):
            ...
            # BUGFIX: its unclear why this index out of range error would occur
            if x == len(ax):
                return ''
            return ax[x]
        else:
    ...
    def load_more_data(self, value, rows=False, columns=False):
        ...
        try:
            if rows and value == self.verticalScrollBar().maximum():
                ...
                """ blocking this as this would require new filter header
